...
def play_game(game) :
    ...
    while(game.winner == 0) :  
        ...
        try : 
            game = move(game, current_player.player_id, get_move(game, current_player.player_id))

            db.session.commit() 
        except ValueError as e:
            raise e
            
    end_game(game,current_player.player_id)
...
def trainning() : 
    ...
    nb_games_played = db.session.query(Game).filter(Game.player_1_id == ai_id, Game.player_2_id == ai_id).count()
